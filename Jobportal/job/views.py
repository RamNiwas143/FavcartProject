    ...
    if request.method == "POST":
        ...
        if user:
            try:
                ...
                if user1.type == "recruiter" and user1.status!="panding":
                    login(request, user)
                    error = "no"
                else:
                    error = "not"
...
def recruiter_signup(request):
    ...
    if request.method == 'POST':
        ...
        try:
            user = User.objects.create_user(first_name=f, username=e, last_name=l, password=p)
...
def user_home(request):
    ...
    st = StudentUser.objects.all().filter()
    d = {'st': st}
...
def recruiter_panding(request):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    data = Recruiter.objects.filter(status='panding')
    d = {'data':data}
    return render(request,'recruiter_panding.html',d)
...
def contactus(request):
    if request.method == 'POST':
        ...
        contact(name=name, email=email, msg=msg,phone=mobile).save()
...
def user_latestjobs(request):
    job = Job.objects.all().order_by('-start_date')
    user = request.user
    student = StudentUser.objects.all().get(user=user)
